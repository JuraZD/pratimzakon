...
Koristi PostgreSQL (SQLAlchemy) umjesto SQLite.
"""
...
class NarodneNovineScraper:
    ...
    def scrape_nn_broj(self, godina: int, broj: int) -> List[Dict]:
        """Preuzima dokumente za određeni NN broj."""
        try:
            params = {
            ...
            logging.info(f"NN {broj}/{godina}: {len(results)} zapisa")
            return results
    ...
    def get_latest_nn_broj(self, db_session=None) -> Tuple[int, int]:
        ...
        logging.info(f"Najnoviji NN broj: {latest_found}/{godina}")
