...
PLAN_CONFIG = {
    ...
    "basic": {"subscription_status": "active",  "plan_type": "basic",  "keyword_limit": 10},
    "plus":  {"subscription_status": "active",  "plan_type": "plus",   "keyword_limit": 20},
}


@router.post("/set-plan")
def set_plan(data: SetPlanRequest, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    if data.plan not in PLAN_CONFIG:
        raise HTTPException(status_code=400, detail="Neispravan plan. Dozvoljeni: free, basic, plus")
    ...
    db.add(Log(event_type="plan_set", user_id=user.id, detail=f"admin set plan={data.plan} months={data.months}"))
    db.commit()
